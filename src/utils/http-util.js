...
const parseNetworkError = (error) => {
    ...
    if (error.response) {
      ...
      errorResponse = {error: true, result: "error", status: error.response.status, data: error.response.data, headers: error.response.headers};
    } else if (axios.isCancel(error)) {
        // This one is intentionally canceled request
        errorResponse = {error: false, result: "canceled", status: null, data: null, message: error.message, isCancel: true};
    } else {
