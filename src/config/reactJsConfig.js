...
return (dispatch, getState) => {
  ...
  const authToken = state.currentUser.token;
      ...
      },
      {
        ...
        keywords: "axios vs fetch",
        id: "axios_vs_fetch",
        description: "Abort controller is an object used to cancel requests.",
        list: [
          "It has a single method abort() and a single property signal that allows to set event listeners on it.",
          `When abort() is called: controller.signal emits the "abort" event and controller.signal.aborted property becomes true.`,
        ],
        code: [
          {
...
controller.abort()`,
        ...
        note: null,
        embededUrl:
          "https://codesandbox.io/embed/axios-and-fetch-ws6uvp?fontsize=14&hidenavigation=1&theme=dark",
        table: [
          {
            columns: [
              {
            ...
            data: [
              ...
              {
                fetch: "can be read directly by browser",
                axios: "Has to be compiled",
              },
              {
                fetch:
                ...
                axios: "Axios performs automatic transforms of JSON data.",
              },
              {
                fetch:
                  "Fetch, by default, doesn’t provide a way to intercept requests",
                axios: "Axios has the ability to intercept HTTP requests.",
              },
            ],
          },
        ],
